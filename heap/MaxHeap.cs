...
public class MaxHeap<T>(IComparer<T>? comparer) : Heap<T>(comparer)
{
    ...
    protected override void HeapifyUp(int index)
    {
        ...
        while (index > 0 && !Compare(TheHeap[parentIndex], TheHeap[index]))
        {
    ...
    protected override void HeapifyDown(int index)
    {
        while (true)
        {
            ...
            if (leftChild < TheHeap.Count && !Compare(TheHeap[targetChild], TheHeap[leftChild]))
                targetChild = leftChild;
            if (rightChild < TheHeap.Count && !Compare(TheHeap[targetChild], TheHeap[rightChild]))
